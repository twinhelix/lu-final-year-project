 ...
 */
public class GAExpertSmartEventListner implements NSGA2Listener
{
	...
	public void performNSGA2Event(NSGA2Event nsga2event)
	{
		if (nsga2event.getNumberGeneration() % 2 == 0)
		{
			...
			LinkedList<GAIndividualSmartCrossover> bestAssignments = new LinkedList<GAIndividualSmartCrossover>();
			for (Individual individual : bestIndividuals)
			{
				bestAssignments.add((GAIndividualSmartCrossover) individual);
			}
	...
	private static void printBestAssignments(
			LinkedList<GAIndividualSmartCrossover> bestAssignments)
			throws FileNotFoundException, IOException
	{
		...
		GAIndividualSmartCrossover[] array = bestAssignments
				.toArray(new GAIndividualSmartCrossover[bestAssignments.size()]);
		...
		for (int i = 0; i < array.length && i < solutionSize; i++)
		{
			...
			ReadWriteTextFile.setContents(WRITEFILE, "GA Fitness: "
	...
	private static class GAIndividualComparator implements
			Comparator<GAIndividualSmartCrossover>
	{
		 ...
		 */
		public int compare(GAIndividualSmartCrossover individual1,
				GAIndividualSmartCrossover individual2)
		{
			if (individual1 == null)
			{
				throw new IllegalArgumentException(
